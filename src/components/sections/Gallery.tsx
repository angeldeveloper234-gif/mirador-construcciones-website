        ...
        <section id="gallery" className="py-32 bg-[#0A0909] relative overflow-hidden">
            <div className="container mx-auto px-6">
                <div className="mb-20">
                    ...
                    <h2 className="text-4xl md:text-6xl font-stardom text-white tracking-widest uppercase">
                        PROYECTOS <span className="text-[#FF4A25]">RECIENTES</span>
                    </h2>
                </div>
                    ...
                    {projects.slice(0, 8).map((item, i) => (
                        ...
                        >
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 p-8 flex flex-col justify-end">
                                <span className="text-[#FF4A25] text-[10px] uppercase tracking-[0.3em] font-black mb-2">
                                    {item.category}
                                </span>
                                <h4 className="text-white font-stardom text-2xl tracking-widest uppercase">
                                    {item.title}
                                </h4>
                            </div>

                            {/* Corner detail */}
                            <div className="absolute top-4 right-4 w-8 h-8 border-t border-r border-white/20 group-hover:border-[#FF4A25] transition-colors duration-500" />
                        </motion.div>
                    ))}
                </div>
            </div>
